...
// Timer sau chia prescaler còn 100kHz
#define TIM_CLK_HZ   100000.0f
/* USER CODE END PM */
...
uint64_t GetMicros64(void) {
	return (uint64_t) tim4_overflow_count * 65536ULL
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM2 && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)
    {
        ...
        else
        {
            ...
            if (ic_val2 > ic_val1)
                delta = ic_val2 - ic_val1;
            else if (ic_val2 < ic_val1)
                delta = (0xFFFF - ic_val1 + ic_val2 + 1);
            else
                delta = 1; // tránh chia 0

            // Tính tần số & RPM
            float freq = TIM_CLK_HZ / (float)delta;  // Hz
            rpm = freq * 60.0f;                      // vì 1 xung = 1 vòng
